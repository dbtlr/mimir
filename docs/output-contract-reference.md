...
- `annotations` — freeform in-flight notes. Structured formats (`json`/`jsonl`) always carry the full array (`content` + `createdAt`, chronological); the styled `records` view stays a compact count (`annotations  3`) unless `--col annotations` is given, which expands every entry with its timestamp and content, mirroring how `history` expands its own row (MMR-361).
...
CLI, an absent parameter reads the whole vault because the daemon has no bound
scope.
