...
| seed · seeds _(`MMR-245`)_                  | `mimir seed "<title>" -k <kind> [-p KEY] [--desc …]` files a grooming-queue seed (`KEY-sN`; target + requester default from the bound board) · `mimir seeds [-p KEY] [--requester KEY] [--status <s>] [--sort asc\|desc] [--grouped]` reads the queue (live + oldest-first by default; `--grouped` = the lane view). Echoes the seed record (`-f ids` prints the `KEY-sN`) |
| promote _(`MMR-245`)_                       | `mimir promote KEY-sN --parent <node> [--priority --size --desc --title --tag…]` creates a task from the seed (or `--link KEY-seq` records existing work), appends the `spawned` link, and moves `new → promoted` (repeatable while promoted; `--parent`/`--link` are mutually exclusive)                                                                                  |
...
| update KEY-sN _(`MMR-245`)_                 | `mimir update KEY-sN [--title --kind --desc]` patches a live seed (the id grammar routes s-ids to the seed store); `resolved`/`rejected` seeds refuse patches. `--upstream KEY-sN` on `create task` / `update KEY-seq` sets the requester-side seed pointer                                                                                                                |
