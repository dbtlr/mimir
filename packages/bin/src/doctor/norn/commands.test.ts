...
import type { DoctorBackend } from '../contract';
    ...
    readSnapshot: () =>
      Promise.resolve({
        ...
        documents: docs.map(({ body, projectedProject, stem }) => ({
          body,
          documentHash: null,
          frontmatter: { project: projectedProject, updated_at: '2026-01-01T00:00:00.000Z' },
          path: `${stem.split('-')[0] ?? stem}/${stem}.md`,
          stem,
        })),
        graph: { nodes: [], projectKeys: [] },
...
test('doctor ITSELF failing (the vault read throws) propagates, not a swallowed exit 0', async () => {
  ...
  const io = fakeIo();
  const unreachable = createNornDoctorBackend({
    readRaw: () => Promise.resolve([]),
