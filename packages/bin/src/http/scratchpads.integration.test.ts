...
describe.skipIf(!NORN)('/api/scratchpads', () => {
  ...
  beforeEach(async () => {
    ...
    const initiative = await createInitiative(store, { projectId, title: 'Scratchpad arc' });
    const initiativeId = await nodeIdOf(store, `MMR-${String(initiative.seq)}`);
    const phase = await createPhase(store, { parentId: initiativeId, title: 'HTTP' });
    const phaseId = await nodeIdOf(store, `MMR-${String(phase.seq)}`);
    const task = await createTask(store, { parentId: phaseId, title: 'Parity' });
  ...
  test('serves every working mutation, strict input, scoped list, and Overview parity', async () => {
    ...
    expect(receipt).toMatchObject({ open_agenda: 0, project: 'MMR', title: 'HTTP parity' });

    await send('POST', '/api/scratchpads', { project: 'NRN', title: 'Other board' });
    ...
    const stale = await send('POST', `/api/scratchpads/${id}/checkpoints`, {
      content: 'stale write',
      expected_updated_at: createdResponse.headers.get('date') ?? 'stale',
    });
    ...
    expect(detail).toMatchObject({
      agenda: [
        ...
        { content: 'replace this', number: 2, reason: 'better path', state: 'superseded' },
      ],
  ...
  test('refuses unsafe discard and permits an explicit forced discard', async () => {
    let receipt = await parse(
      await send('POST', '/api/scratchpads', { project: 'MMR', title: 'Discard me' }),
    );
    const id = String(receipt.id);
    receipt = await parse(
      await send('POST', `/api/scratchpads/${id}/agenda`, {
  ...
  test('surfaces a staged freeze and completes it on retry after artifact failure', async () => {
    ...
    const created = await createScratchpadService(store.scratchpads, store.artifacts).create({
      project: 'MMR',
    ...
    expect(await parse(recovered)).toMatchObject({ id: 'MMR-a1', summary: 'retry me' });
  });
});
