...
test('seq is per-project, monotonic, and shared across node types', async () => {
  ...
  const q = await createProject(store, { key: 'NRN', name: 'Norn' });
  const init2 = await createInitiative(store, { projectId: q.id, title: 'i2' });
...
test('createPhase requires an initiative parent and inherits project_id', async () => {
  const p = await createProject(store, { key: 'MMR', name: 'Mimir' });
  const init = await createInitiative(store, { projectId: p.id, title: 'i' });
  const phase = await createPhase(store, { parentId: init.id, target: 'ship it', title: 'ph' });
  ...
  await expectMimirError('validation', () =>
    createPhase(store, { parentId: phase.id, title: 'no' }),
  );
...
test('createTask sets both axes, ranks at append step, and accepts phase or initiative parents', async () => {
  ...
  const t2 = await createTask(store, { parentId: phase.id, title: 't2' });
  expect(t2.rank).toBe(RANK_STEP * 2); // appends below t1

  // a task directly under an initiative is allowed (phaseless initiative)
  const t3 = await createTask(store, { parentId: init.id, title: 't3' });
  expect(t3.parent_id).toBe(init.id);

  // a task under a task is rejected
  await expectMimirError('validation', () => createTask(store, { parentId: t1.id, title: 'no' }));
});
