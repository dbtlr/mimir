...
test.skipIf(!NORN)('tag and untag round-trip over MCP, reaching project and node', async () => {
  ...
  const parsed = parseJson<{ tags: { tag: string }[] }>(textOf(view));
  // A tag application carries no note on any entity (ADR 0005 Refinement, MMR-270).
  expect(parsed.tags.map((t) => ({ tag: t.tag }))).toEqual([{ tag: 'spec' }]);
