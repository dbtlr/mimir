...
describe('nodeDossier', () => {
  ...
  it('surfaces the description body and carried-forward meta rows', async () => {
    ...
    // suite pins TZ, so 10:00Z is 06:00 EDT, never the raw UTC instant. The
    // gap before AM/PM is whatever space this ICU emits (U+202F since ICU 72),
    // so match it loosely: the reading is the assertion, not the separator.
    expect(screen.getAllByText(/^6\/1\/2026, 6:00:00\sAM EDT$/)).toHaveLength(2);
    expect(screen.getByText(/^6\/30\/2026, 8:00:00\sPM EDT$/)).toBeDefined();
