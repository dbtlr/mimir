      ...
      <header className="flex items-start gap-3 border-b border-line p-4 pb-3">
        ...
        <div className="flex min-w-0 flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="font-mono text-[11px] text-ink-dim">{id}</span>
            <Badge variant="outline">{artifact.data?.project ?? "…"}</Badge>
          </div>
          <h1 className="text-[15px] leading-snug font-semibold text-ink-bright">
            ...
            <div className="flex flex-wrap gap-1.5">
              {artifact.data.links.map((nodeId) => (
                <button
                  ...
                  className="rounded-[3px] px-1.5 py-0.5 font-mono text-[11px] text-accent transition-colors hover:bg-well-800 focus-visible:outline-2 focus-visible:outline-accent"
                >
                  {nodeId}
                </button>
              ))}
