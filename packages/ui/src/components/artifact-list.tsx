 ...
 * demoted (opacity in dark, an ink tier down in light — ADR 0019 §7 rule 3).
 */
    ...
    <div className="flex flex-col gap-1 px-2 pb-2">
      {groupByRecency(items).map((group) => (
        <section key={group.label} aria-label={group.label}>
          <h2 className="microlabel px-2 pt-2.5 pb-1 font-mono text-ink-faint">{group.label}</h2>
                ...
                <li key={a.id}>
                  <button
                    ...
                    className={cn(
                      ...
                      !selected && !group.recent && 'dark:opacity-75',
                    )}
                  >
                    <span
                      className={cn(
                        ...
                        // Light demotes by ink tier, not opacity (ADR 0019 §7 rule 3).
                        !selected && !group.recent && 'light:text-ink',
                      )}
                    >
                      {a.title}
                    </span>
