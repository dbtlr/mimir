...
- **`mimir doctor` detects non-canonical stored timestamps, and `--fix` repairs
  ...
  instant — an offset zone, an absent millisecond, extra precision — is an
  `error` finding (`non-canonical-timestamp`) that `--fix` rewrites under a
  compare-and-set on the observed value. A **zone-less or malformed** value
