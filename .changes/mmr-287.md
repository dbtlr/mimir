...
- **The mimir skill teaches the settled CLI taxonomy** (MMR-287). The stale
  "no per-command help" claim is gone — `mimir <verb> -h`/`--help` has been
  real since the COMMAND_HELP registry landed — replaced with the actual
  contract (per-command help is a flag reminder; the references remain the
  authority to drive from). The skill now carries
